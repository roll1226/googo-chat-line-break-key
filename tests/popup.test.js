...
describe("loadSetting", () => {
  ...
  it("only checks one radio at a time", () => {
    ...
    const checked = [
      ...document.querySelectorAll('input[name="lineBreakKey"]'),
    ].filter((r) => r.checked);
...
describe("syncSelectedClass", () => {
  ...
  it('removes "selected" from unchecked option parents', () => {
    ...
    expect(unchecked.closest(".option").classList.contains("selected")).toBe(
      false,
    );
  });
...
describe("updatePlatformLabels", () => {
  ...
  it("updates labels to ⌘/⌥ on Mac", () => {
    ...
    expect(document.getElementById("ctrl-enter-label").textContent).toBe(
      "⌘ + Enter",
    );
    expect(document.getElementById("alt-enter-label").textContent).toBe(
      "⌥ + Enter",
    );
  });
